...
io.on('connection', (socket) => {
    ...
    try {
      ...
      if (fileUrl && fileUrl.startsWith('data:')) {
        ...
        if (matches && matches.length === 3) {
          ...
          const uniqueKey = 'PipeWyze/' + Date.now().toString() + '.' + extension;
  ...
  socket.on('start_upload', ({ uploadId, roomId, senderId, fileName, fileType, totalChunks }) => {
  ...
  socket.on('upload_chunk', async ({ uploadId, chunkIndex, chunkData }) => {
    ...
    try {
      ...
      if (upload.receivedChunks === upload.totalChunks) {
        ...
        upload.writeStream.on('finish', async () => {
          try {
            ...
            socket.emit('upload_error', { uploadId, error: 'Failed to process completed file' });
      ...
      socket.emit('upload_error', { uploadId, error: 'Failed to write file chunk' });
    }
  });
