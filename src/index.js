...
mongoose.connect(config.mongoose.url).then(() => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('send_message', async ({ roomId, senderId, receiverId, content, fileUrl, fileType, fileName }) => {
      try {
        if (!roomId || !senderId || (!content && !fileUrl)) {
          console.warn('Validation Failed: missing roomId, senderId, or both content and fileUrl.');
          return;
        }
    ...
    socket.on('start_upload', async ({ uploadId, roomId, senderId, fileName, fileType, fileSize, totalChunks }) => {
      ...
      console.log('Metadata:', { uploadId, roomId, senderId, fileName, fileType, fileSize, totalChunks });
      ...
      if (!ALLOWED_MIME_TYPES.includes(fileType)) {
        socket.emit('upload_error', { uploadId, error: 'Unsupported file type' });
        return;
      }
      ...
      try {
        ...
        if (room.homeOwnerId.toString() !== senderId && room.plumberId.toString() !== senderId) {
          socket.emit('upload_error', { uploadId, error: 'Access denied: You are not authorized in this room' });
          return;
        }
        ...
        if (existing) {
          ...
          if (fs.existsSync(existing.tempFilePath)) {
            try { fs.unlinkSync(existing.tempFilePath); } catch (e) {}
          }
        ...
        const timeoutId = setTimeout(() => {
          ...
          if (up) {
            ...
            if (fs.existsSync(up.tempFilePath)) {
              try { fs.unlinkSync(up.tempFilePath); } catch (e) {}
            }
            activeUploads.delete(uploadId);
        ...
        socket.emit('upload_error', { uploadId, error: 'Internal server error' });
      }
    });
    ...
    socket.on('upload_chunk', async ({ uploadId, chunkIndex, chunkData }, callback) => {
      ...
      const sendAck = () => {
        ...
        if (upload.fileSize) {
          progressPayload.receivedBytes = upload.receivedChunks * (upload.fileSize / upload.totalChunks);
      ...
      const handleFail = (errorMsg) => {
        if (upload) {
          ...
          if (fs.existsSync(upload.tempFilePath)) {
            try { fs.unlinkSync(upload.tempFilePath); } catch (e) {}
          }
          activeUploads.delete(uploadId);
        }
      ...
      if (upload.chunksWritten.has(idx)) {
        console.warn(`Duplicate chunk index received: ${idx} for upload ${uploadId}. Skipping.`);
        sendAck();
        return;
      }

      if (idx !== upload.expectedChunkIndex) {
        handleFail(`Out of order chunk received. Expected: ${upload.expectedChunkIndex}, got: ${idx}`);
        return;
      }
        ...
        const processCompleteUpload = () => {
          ...
          upload.writeStream.on('finish', async () => {
            try {
              ...
              if (stats.size === 0 || (upload.fileSize && stats.size > upload.fileSize)) {
                throw new Error('Actual file size does not match metadata / is zero');
              }
              ...
              await s3.send(command);
              console.log(`Successfully uploaded file to S3. Key: ${uniqueS3Key}`);
        ...
        if (upload.socketId === socket.id) {
          ...
          if (fs.existsSync(upload.tempFilePath)) {
            try {
              ...
              console.error(`Failed to delete temp file ${upload.tempFilePath}:`, err);
            }
          }
          activeUploads.delete(uploadId);
        }
