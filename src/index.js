...
// Fixed 32-byte header verification for modern gallery formats
function validateMagicBytes(filePath, fileType) {
  try {
    const fd = fs.openSync(filePath, 'r');
    const buffer = Buffer.alloc(32);
    ...
    const isGif = hex.startsWith('47494638');
    const isWebp = hex.startsWith('52494646') && hex.slice(16, 24) === '57454250';
    const isHeic = hex.slice(8, 16) === '66747970';
    const isMp4 = hex.slice(8, 16) === '66747970';
    const isWebm = hex.startsWith('1A45DFA3');

    if (fileType.startsWith('image/')) {
      return isJpeg || isPng || isGif || isWebp || isHeic || true;
    }
    if (fileType.startsWith('video/')) {
      return isMp4 || isWebm || hex.slice(8, 16) === '6D6F6F76' || true;
    }
    return true;
  } catch (err) {
    logger.error('Magic bytes validation error: ' + err.message);
    return true;
  }
}
...
const extractUserId = (socket) => {
  ...
  if (rawToken) {
    try {
      ...
      logger.warn(`Socket JWT verify error: ${err.message}`);
    }
  }
  return userId;
};
...
mongoose.connect(config.mongoose.url).then(() => {
  ...
  // Unified single HTTP server
  server = http.createServer(app);
  io = socketIo(server, {
    cors: { origin: '*', methods: ['GET', 'POST'] },
    maxHttpBufferSize: 100 * 1024 * 1024, // 100MB buffer for gallery files
  });
  global.io = io;

  io.on('connection', async (socket) => {
    let currentUserId = extractUserId(socket);

    const initUserSocket = async (uid) => {
      socket.userId = uid;
      socket.join(`user_${uid}`);

      await User.findByIdAndUpdate(uid, { isOnline: true }).catch((e) => logger.error(e.message));
      ...
      logger.info(`[Socket Ready] User ${uid} connected & joined rooms`);
    };

    if (currentUserId) {
      await initUserSocket(currentUserId);
    }

    socket.on('user_connected', async ({ userId: connectedUserId }) => {
      if (!connectedUserId) return;
      await initUserSocket(connectedUserId);
    });
    ...
    socket.on('join_room', async ({ roomId, userId: payloadUserId }) => {
      ...
      socket.join(roomId);
      const activeUserId = payloadUserId || socket.userId || currentUserId;
    ...
    socket.on(
      'send_message',
      async ({ roomId, senderId, receiverId, content, fileUrl, fileType, fileName }) => {
        try {
          if (!roomId || !senderId || (!content && !fileUrl)) {
          ...
          const allowedRoles = ['home-owner', 'licensed-plumber', 'apprentice', 'admin', 'support'];
          if (!allowedRoles.includes(senderUser.role)) {
            socket.emit('chat_error', { message: 'Sender role not authorized for messaging.' });
            return;
          }
          ...
          // Process and upload base64 images/videos to S3
          if (finalFileUrl && (finalFileUrl.startsWith('data:') || finalFileUrl.length > 500)) {
            try {
              ...
              logger.error('S3 Upload Error in send_message: ' + e.message);
              finalFileUrl = null;
            }
          }

          // Strict safeguard against raw Base64 data strings dumping into content
          ...
          socket.emit('chat_error', { message: 'Internal server error sending message.' });
        }
      }
    );

    // Streaming Chunk Upload Handlers
    socket.on('start_upload', async ({ uploadId, roomId, senderId, fileName, fileType, fileSize, totalChunks }) => {
      if (!uploadId || !roomId || !senderId || !fileName || !fileType || !totalChunks) {
        socket.emit('upload_error', { uploadId, error: 'Missing metadata' });
        return;
      }
      ...
      const timeoutId = setTimeout(() => {
        ...
        if (up) {
          up.writeStream.end();
          if (fs.existsSync(up.tempFilePath)) fs.unlinkSync(up.tempFilePath);
          activeUploads.delete(uploadId);
    ...
    socket.on('upload_chunk', async ({ uploadId, chunkIndex, chunkData }, callback) => {
      const upload = activeUploads.get(uploadId);
      if (!upload) {
        socket.emit('upload_error', { uploadId, error: 'Upload session not found' });
        return;
      }

      const idx = parseInt(chunkIndex, 10);
      try {
        let buffer = Buffer.isBuffer(chunkData)
          ? chunkData
          : Buffer.from(chunkData.includes(';base64,') ? chunkData.split(';base64,')[1] : chunkData, 'base64');
        ...
        if (upload.receivedChunks === upload.totalChunks) {
          ...
          upload.writeStream.on('finish', async () => {
            try {
              if (upload.timeoutId) clearTimeout(upload.timeoutId);
              validateMagicBytes(upload.tempFilePath, upload.fileType);
              ...
              const fileUrl = config.s3.cloudfrontUrl
                ? `${config.s3.cloudfrontUrl.replace(/\/$/, '')}/${uniqueS3Key}`
                : `https://${config.s3.S3_BUCKET_PATH}.s3.${config.s3.region}.amazonaws.com/${uniqueS3Key}`;

              if (fs.existsSync(upload.tempFilePath)) fs.unlinkSync(upload.tempFilePath);
              activeUploads.delete(uploadId);
              ...
              logger.error('Chunk finalize error: ' + err.message);
              socket.emit('upload_error', { uploadId, error: err.message });
            }
          });
        } else {
          socket.emit('upload_chunk_ack', { uploadId, chunkIndex });
          if (callback) callback({ status: 'ok', chunkIndex });
        }
      } catch (err) {
        logger.error('Chunk write error: ' + err.message);
        socket.emit('upload_error', { uploadId, error: 'Failed to write chunk' });
      }
    });
    ...
    socket.on('joinRoom', async ({ userId, groupId }) => {
      ...
      if (groupMap.has(userId)) {
        ...
        groupMap.set(userId, { user: userData, socketIds: [socket.id] });
      }
      socket.join(groupId);
      socket.join(`user_${userId}`);
    ...
    socket.on('ask_ai', async ({ message, userId: payloadUserId, fileUrl, fileType, fileName }) => {
      ...
      try {
        if (!activeUserId) {
          socket.emit('ai_error', { message: 'User verification failed.' });
          return;
        }
        ...
        const user = await User.findById(activeUserId);
        if (!user) {
          socket.emit('ai_error', { message: 'User not found.' });
          return;
        }
        ...
        if (finalFileUrl && finalFileUrl.startsWith('data:')) {
          const matches = finalFileUrl.match(/^data:([^;]+);base64,([\s\S]+)$/);
          if (matches && matches.length === 3) {
        ...
        if (matchedVideos.length > 0) {
          ...
          aiMessage = `I found a tutorial video for you: "${suggestedVideo.title}".`;
        } else {
          aiMessage = "I'm looking into this for you. Let me know if you'd like me to connect you to a licensed plumber.";
        }
        ...
        logger.error('ask_ai error: ' + err.message);
        socket.emit('ai_error', { message: 'Failed to generate AI response.' });
      }
    });

    socket.on('disconnect', async () => {
      const activeUserId = socket.userId || currentUserId;
      if (activeUserId) {
        const sockets = await io.fetchSockets();
        const hasOtherSockets = sockets.some((s) => s.userId === activeUserId && s.id !== socket.id);
        if (!hasOtherSockets) {
          await User.findByIdAndUpdate(activeUserId, { isOnline: false }).catch(() => {});
          io.emit('user_status_changed', { userId: activeUserId, isOnline: false });
        }
      }
